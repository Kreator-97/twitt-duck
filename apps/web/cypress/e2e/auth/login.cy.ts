...
describe('tests integration on /auth/', () => {
  ...
  const emailTest = 'testcypress01@test.com'
  ...
  it('should to show an error when username is not availible on register', () => {
    cy.contains(' Crea una cuenta').click()
    cy.get('input[type="text"]').type('test cypress 01')
    cy.get('input[type="email"]').type('any-test-email@test.com')
    cy.get('input[type="password"]').type('12345678')
    cy.contains('Continuar').click()
  ...
  after(() => {
    cy.request('DELETE', `http://localhost:5000/api/user/test/${emailTest}`)
    cy.request('DELETE', 'http://localhost:5000/api/user/test/any-test-email@test.com')
  })
})
